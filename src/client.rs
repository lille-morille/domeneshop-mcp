...
/// * `http` + `base_url` — raw `reqwest` client used by resources so we can
///   forward Domeneshop's JSON verbatim. This sidesteps upstream quirks where
///   the real API returns integers as strings (MX `priority`, SRV
...
pub fn build(authorization: HeaderValue, base_url: &str) -> Result<ApiClient, McpError> {
